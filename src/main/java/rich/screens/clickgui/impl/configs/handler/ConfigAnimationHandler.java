...
public class ConfigAnimationHandler {
    ...
    // Улучшенные скорости анимаций
    private static final float PANEL_ALPHA_SPEED = 18f; // Быстрее
    private static final float PANEL_SLIDE_SPEED = 22f; // Быстрее
    private static final float CREATE_BOX_SPEED = 16f;
    private static final float ITEM_APPEAR_SPEED = 24f; // Быстрее появление
    private static final float HOVER_SPEED = 10f; // Быстрее hover
    ...
    private void updatePanelAnimations(boolean isActive, float deltaTime) {
        ...
        panelAlpha += alphaDiff * applySpring(PANEL_ALPHA_SPEED * deltaTime);
        panelAlpha = Math.max(0f, Math.min(1f, panelAlpha));

        float targetSlide = isActive ? 1f : 0f;
        float slideDiff = targetSlide - panelSlide;
        panelSlide += slideDiff * applySpring(PANEL_SLIDE_SPEED * deltaTime);
    ...
    private void updateCreateBoxAnimation(boolean isCreating, float deltaTime) {
        ...
        createBoxAnimation += (targetCreate - createBoxAnimation) * applySpring(CREATE_BOX_SPEED * deltaTime);
    }
    ...
    private void updateItemAnimations(boolean isActive, List<String> configs, float deltaTime) {
        ...
        for (String config : configs) {
            ...
            if (isActive) {
                float delay = index * 0.015f; // Чуть быстрее задержка
                if (panelAlpha > delay) {
                    targetAppear = 1f;
                } else {
                    targetAppear = 0f;
            ...
            float speed = isActive ? ITEM_APPEAR_SPEED : 18f;
            float appearDiff = targetAppear - currentAppear;
            currentAppear += appearDiff * applySpring(speed * deltaTime);
    ...
    private void updateHoverAnimations(List<String> configs, float deltaTime) {
        for (String config : configs) {
            ...
            hoverAnimations.put(config, current + (0f - current) * applySpring(HOVER_SPEED * deltaTime));

            float deleteCurrent = deleteHoverAnimations.getOrDefault(config, 0f);
            deleteHoverAnimations.put(config, deleteCurrent + (0f - deleteCurrent) * applySpring(HOVER_SPEED * deltaTime));

            float loadCurrent = loadHoverAnimations.getOrDefault(config, 0f);
            loadHoverAnimations.put(config, loadCurrent + (0f - loadCurrent) * applySpring(HOVER_SPEED * deltaTime));

            float refreshCurrent = refreshHoverAnimations.getOrDefault(config, 0f);
            refreshHoverAnimations.put(config, refreshCurrent + (0f - refreshCurrent) * applySpring(HOVER_SPEED * deltaTime));
        }
    }

    /**
     * Применяет SPRING easing к прогрессу анимации
     */
    private float applySpring(float rawProgress) {
        return (float) Easings.SPRING.ease(Math.min(1.0, rawProgress));
    }

    public void updateSelectedAnimation(boolean hasSelection, float deltaTime) {
        float targetSelected = hasSelection ? 1f : 0f;
        selectedAnimation += (targetSelected - selectedAnimation) * applySpring(10f * deltaTime);
    }
